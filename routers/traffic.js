...
router.post('/audit', function(req, res) {
  ...
  async.parallel(
      ...
      function(err, results){
          ...
          async.parallel(
              {
                  '100 m': (audit.calculate).bind({ results: results, distance: '100 m', population: results['population_100m']['features'][0]['properties']['population'], center: [lon, lat], weight: 0.5 }),
                  '500 m': (audit.calculate).bind({ results: results, distance: '500 m', population: results['population_500m']['features'][0]['properties']['population'], center: [lon, lat], weight: 0.3 }),
                  '1 km': (audit.calculate).bind({ results: results, distance: '1 km', population: results['population_1km']['features'][0]['properties']['population'], center: [lon, lat], weight: 0.15 }),
                  '5 km': (audit.calculate).bind({ results: results, distance: '5 km', population: results['population_5km']['features'][0]['properties']['population'], center: [lon, lat], weight: 0.05 })
              },
              function(err, scores){
                  if(err){
                      console.log(err);
